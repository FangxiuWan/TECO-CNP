...
! Step 1, cal
  ...
  !1 -- calculate the soil water 
      ...
      !  write(*,*)'wcltot_rec',sum(wcltot_rec(:))/8760,itime,ispinup
      !  write(*,*)'wsctot_rec',sum(wsctot_rec(:))/8760,itime,ispinup
      !ENDIF
  ...
  !5 -- calculate the diffusion flux
      ...
      !write(*,*)'FdiffP,D,delta_QLabP_root,QPlab',FdiffP,D,delta_QLabP_root,QPlab
  !6 -- calculate the fraction of P concentration at root surface with F_diffP
      ...
      !write(*,*)'a3',f_rootp,f_rootp*QPlab+FdiffP,QPlab
      !stop
...
SUBROUTINE SCALAR_PN(f_PNplant)
      ...
      !write(*,*)'pn_plant',1/pn_plant,NSN,NSP
...
MODULE NutrientsUptake
  ...
  SUBROUTINE N_UptakeDemand(NSNmax,NSNmin,Nfix0,&
    ...
    If(N_demand > 0.0)THEN   !add by wan, for debug
      ...
      !ELSE
        !write(*,*)'N_fixation_x,Cfix',Cfix,Cuptake
          ! Nitrogen fixation
  ...
  !     Total C cost for nitrogen   
    ...
    IF(itime .EQ. 8760 .and. MCMC .EQ. 0) THEN
      write(*,*)'N_deficit_rec,N_demand_rec,N_uptake_rec,N_fixation_x_rec',&
      & sum(N_deficit_rec(:)),sum(N_demand_rec(:)),sum(N_uptake_rec(:)),sum(N_fixation_x_rec(:))
  ...
  ! same flow with N cycle
    ...
    !write(*,*)'P_demand,NPP_L,W,R,Re',P_demand,NPP_L,NPP_W,NPP_R,NPP_Re
    !write(*,*)'CP1234,ispinup,itime',CP(1),CP(2),CP(3),CP(4),ispinup,itime
    !write(*,*)'1',NPP_L/CP(1),NPP_W/CP(2),NPP_R/CP(3),NPP_Re/CP(4)
    !write(*,*)'2',NPP_L/CP(1)+NPP_W/CP(2)+NPP_R/CP(3)+ NPP_Re/CP(4),P_deficit
    !stop
    ...
    IF(P_demand > 0.0)THEN   
        ...
        !write(*,*)'rootsurfaceP',rootSurfaceP,f_rootp,QPlab
        ...
        ! MaxUpCapP = 8.E-6
        ! QPlab = 0.3
        ! write(*,*)'MaxUpCapP',MaxUpCapP,rootSurfaceP,f_rootp,QPlab
        !f_PNplant = 0.02
        P_uptake = AMIN1 (MAX(0.0, P_demand+P_deficit),MaxUpCapP*(QC(3)/fbmC)*S_t(1)*f_PNplant)
        !write(*,*)'f_PNplant',f_PNplant
        ...
        !write(*,*)'bP,MaxUpCapP,QC(3),S_t(1),f_PNplant,P_demand',&
        !              bP,MaxUpCapP,QC(3),S_t(1),f_PNplant,P_demand
        
        ! calculate the limitation factor - LfactorP
        CALL xnp_soil()

        ! ensure soil P min doesn't get depleted
        !IF (P_uptake .gt. P_miner2) THEN
        !      P_uptake=P_miner2  ! From CABLE!!!! WAN
        !      LfactorP = P_miner2/aP
             !write(*,*)'here'
        !sENDIF
        P_demand_rec(itime) = P_demand
        P_demand_rec_test(itime) = P_demand_test
        P_deficit_rec(itime) = P_deficit
        !write(*,*)'P_demand,P_deficit,P_uptake',P_demand,P_deficit,P_uptake,itime,ispinup
        !write(*,*)'P_deficit_sum',sum(P_deficit_rec(1:itime))
        ...
        ! (7) update the reduction fraction of [P] again
          !write(*,*)'1-f_rootp,QPlab,P_uptake',f_rootp,QPlab,P_uptake
        f_rootp = (f_rootp*QPlab-P_uptake)/QPlab
        !write(*,*)'2-f_rootp,QPlab,P_uptake',f_rootp,QPlab,P_uptake
    ...
    !write(*,*)'LfactorP',LfactorP,bP/aP,ispinup,itime,P_deficit
    !IF(bP/aP .gt. 1)write(*,*)'huhuhuhuhuhuhuhuh',ispinup,itime
