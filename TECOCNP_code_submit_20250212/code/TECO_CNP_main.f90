...
! ------------------------------------------------------------------------------
!
! TECO-CNP_main.f90: Implements the coupled C-N-P cycle for terrestrial ecosystems.
! Included Modules:
...
! Version Date: February 12 2025
    ...
    IF (MCMC.eq.0) THEN ! START BIOGEOCHEMICAL CYCLE SIMULATION
        ...
        ispinup = ispinup + 1
        !1- regular simulation, including the traceability analysis module
        CALL CNP_simu(upgraded)          
        !2- alternative section - sensitivity analysis
...
! Version Date: February 12 2025
    ...
    REAL ET,G,wind,eairp,esat,rnet
    REAL Esoil
    ...
    ! for phenology
    REAL totlivbiom
    REAL L_fall,L_add,litter,seeds,S_fall,R_fall ! add by zhoujian: S_fall and R_fall
    REAL stor_use
    ! respiratiom
    ...
    !REAL input_value
    !for SASU
    REAL criteria,delta_V,criteria1,delta_V1,criteria3,delta_V3
    !INTEGER Days,Hours
...
!       Reset the variables each year
        ...
        NSCmax=0.01*(StemSap+RootSap+QC(1)) !wan- change 0.05 to 0.005 for Tiantong 20230322,change to 0.08
        !write(*,*)'NSCmax',NSCmax
        ...
        ENDIF     
!------------------------------------------------
!       ADD by wan- select the situation depends on which cycle is
        ! Another limitation scheme

        ! CYCLE_CNP condition in xnp
        ...
        !Dair=esat(Tair)-eairP                  ! wan- commented out, since we have VPD in TianTong
        radsol=AMAX1(radsol,0.01)

        IF(radsol.GT.10.0) THEN
                G=-25.0              !wan- original, -25, see Garratt, pp116, G - the heat flux into soil
        ELSE
                G=20.5
        ENDIF
        Esoil=0.05*radsol
        ...
        CALL canopy(doy,hour,radsol,Tair,Dair,eairP,    &   ! from climate data file,including 
            &              wind,rain,&
            &              Rnet,G,Esoil,&
            &              Tsoil,co2ca,&            !constants specific to soil and plant
...
!   Layer volume (cm3)
    ...
    DO i=1,10
        ...
        !write(*,*)'rdepth,DEPTH',rdepth,DEPTH,nfr
    ENDDO
    IF (nfr.GT.10) nfr=10
...
!   Fraction of SEVAP supplied by each soil layer
    SRDT(I)=EXP(-6.73*(DEPTH(I)-thksl(I)/2.0)/100.0) !/1.987        !wan- origin
!    SRDT(I)=EXP(-4.73*(DEPTH(I)-thksl(I)/2.0)/100.0)/1.987           !wan-TECO.for, DEPTH = root depth
    !WRITE(*,*)'i,SRDT(i),DEPTH(I)-thksl(I)',i,SRDT(i),DEPTH(I)-thksl(I)
    !   SRDT(I)=EXP(-6.73*(DEPTH(I)-thksl(I)/2.0)/100.0)/1.987           !wan- for test, 4.73 to 6.73, wcl little change 
!    SRDT(I)=EXP(- 4.73*(DEPTH(I)-thksl(I)/2.0)/100.0)/1.2 !/1.987   !wan- works
    !WRITE(*,*)'DEPTH(I),thksl(I),SRDT',DEPTH(I),thksl(I),SRDT(i)  
!   SRDT(I)=AMAX1(0.0,SRDT(I)*(wcl(i)-WILTPT)) !*thksl(I))
    Tsrdt=Tsrdt+SRDT(i)  ! to normalize SRDT(i)                     !wan-origin
    !Tsrdt=Tsrdt+SRDT(i)/(i*i)                                        !wan- TECO.for
    !Tsrdt=Tsrdt+SRDT(i)/i                                           !wan-for test,Tsrdt, Total SRDT?
    ENDDO
...
!   Output fwsoil, omega, and topfws
    ...
    !write(*,*)'topfws',topfws
    !wan 02230710 copy from  TECO.for
...
!**********************************************************************
!     plant growth model
SUBROUTINE plantgrowth(Tair,stor_use,SNgrowth,&
    &                L_fall,S_fall,R_fall,& 
    &                RgLeaf,RgStem,RgRoot,RgRe,count,upgraded)
     USE LIMITATION 
     USE IntersVariables
     USE vars_site
...
!     scalars
     ...
     REAL bW,bT
     REAL L_fall,L_add,NL_fall,NL_add,S_fall, R_fall!tauC(3) ! add by zhoujian tauC(3): tau_L, tau_S, tau_R
     REAL alpha_St
     INTEGER i
...
!   WRITE(*,*)'---storage,stor_use',storage,stor_use
    ...
    IF((onset.eq.1).and.(storage.GT.stor_use))THEN 
        ...
        storage=storage-add
        !WRITE(*,*)'1add',add,stor_use,storage
        !stop
    ELSE
        add=0.0
        onset=0
    ...
    IF(accumulation.LT.(NSCmax*0.4))THEN    !accumulation, 2024 01 23
        ...
        !write(*,*)'itime,Stemsap,rootsap',itime,Stemsap,rootsap
        !WRITE(*,*)'store,accumulation',store,accumulation,NSCmax*0.4
    ELSE
        store=0.0  
    ENDIF
    accumulation = accumulation+store !wan- 'accumulation' help judge when to add carbon to store
    !WRITE(*,*)'accumulation',accumulation
    !why use the gddonset
    St = AMAX1(0.0, 1.0-exp(-(Tair-Tcold)/10))  ! wan 0320 change /15 to 10
    !Wan replaced, from Du, H11, unreasonable!
...
!    Sw=AMIN1(0.5,AMAX1(0.333, 0.333+omega))      !Jiang modified June 2016
    ...
    !write(*,*)'GrowthP,GPmax,St,Sw,fnsc',GrowthP,GPmax,St,Sw,fnsc
    ...
    ELSE
        ...
        ExcessC =0.
    ENDIF
    IF (LFactorP .EQ. 0) THEN
        write(*,*)'2LfactorP',LFactorP,itime
    ENDIF

    !write(*,*)'LfactoP,LfactorN,x_leaf_limit',LfactorP,LfactorN,x_leaf_limit
    !write(*,*)'GrowthP,xnpGrowthP',GrowthP,xnpGrowthP
    xnpGrowthP_temp =xnpGrowthP
    
    !WRITE(*,*)'GPmax*St*Sw*fnsc,0.04*NSC,GrowthP',GPmax*St*Sw*fnsc,0.04*NSC,GrowthP
    IF(GrowthP .GT. 200 .or. isnan(GrowthP)) THEN
        WRITE(*,*)'GrowthP,fnsc,nsc',GrowthP,fnsc,nsc
        Flag_Growth = 1
    ENDIF
    !wan- 1,change, w to Sw
    !2, change 0,04NSC, to 0.02 NSC
    !WRITE(*,*)'1,2,',GPmax*St*W*fnsc*(1.-exp(-NSN)),0.04*NSC
    !IF((GPmax*St*W*fnsc*(1.-exp(-NSN))) .GT. (0.04*NSC))WRITE(*,*)'NSC win'
    !WRITE(*,*)'St, W, fnsc',St, W, fnsc
...
!wan-GrowthR always be zero, and GrowthL=GrowthS, so the alphaL_L=alpha_S=0.5, alpha_R=0!
!So I change this part to DU, Try!
...
!          !c2 = 0  --- not use
    ...
    !   GrowthL=GrowthP*b1
    !   !WRITE(*,*)'b1,bL,bR',b1,QCbmL,QCbmR
    !   GrowthS=GrowthP*b2
    !   GrowthR=GrowthP*b3
    !   NPP = GrowthL + GrowthR + GrowthS + add  ! +add / +store?  +add is more reasonalble
      !WRITE(*,*)'GrowthP,c1,c2',GrowthP,c1,c2
      !Write(*,*)'NPP,ADD,GrowthL,GrowthS,GrowthR',NPP,add,GrowthL,GrowthS,GrowthR

    
!--------Arora & Boer
    k_n = 0.5
    ww = 0.36!0.32!0.35!0.25   !origin: 0.8 TECO-CNP Eq.4-6
    eL  = 0.35!0.35  !origin: 0.35
    eS  = 0.05  !origin: 0.05
    eR  = 0.60!0.60  !origin: 0.60
  
    IF(LAI .LE. LAIMIN)THEN
        a_Re=0.0
    ELSE
        a_Re=0.12 ! wan assume a fixed allocation fraction for fecundity 10% of new carbon
    ENDIF

    L = exp(-k_n*LAI)
    W2 = AMAX1(0.0, Amin1(1.,omega))
    !a_S = (eS+ww*(1.0-L))/(1.+ww*(2.0-L-W2))*x_stem_limit
    a_S = (eS*x_stem_limit+ww*(1.0-L))/(1.+ww*(2.0-L-W2))
    a_L = (eL*x_leaf_limit)/(1.+ww*(2.-L-W2))
    !a_R = (eR+ww*(1.-W2))/(1.+ww*(2.-L-W2))*x_root_limit  ! wrong
    !a_R = ((1-eS*x_stem_limit-eL*x_leaf_limit)+ww*(1.-W2))/(1.+ww*(2.-L-W2)) !wan 20240909
    a_R=1-a_L-a_S   

    !write(*,*)'x_stem_limit,x_leaf_limit',x_stem_limit,x_leaf_limit
    ...
    !WRITE(*,*)'xnp_leaf_limit,xNPuptake,NPP',xnp_leaf_limit,xNPuptake,NPP
    ...
    IF(NPP.eq.0.0)THEN
        ...
        alpha_R=0.3
        alpha_Re=0.1
        ! update the organs growth 20240725
        !xnpGrowthL = 0.0
        !xnpGrowthS = 0.0
        !xnpGrowthR = 0.0
        !xnpGrowthRe = 0.0
...
!   Maximum rates of leaf fall induced by low T and drought respectively
    ...
    !gamma_Wmax=0.03/24. ! maxmum leaf fall rate per hour
    !gamma_Tmax=0.03/24.
...
!             gamma_N=0.
!       ENDIF
! wan- I need add the onset to identify the fallen time! 
    ! IF(Tair.GT.(Tcold+10.)) THEN
    !     beta_T=1.
    ! ELSE 
    !     IF(Tair.GT.Tcold .and. onset .eq. 0) beta_T=(Tair-Tcold)/10.
    !     IF(Tair.LE.Tcold) beta_T=0.
    ! ENDIF

    ! Wrong code ！
    !IF(Tair.GE.(Tcold+10.)) THEN  !.and. onset .eq. 1
    !    beta_T=1.
    !    Write(*,*)'1beta_T',beta_T
    !ELSE 
    !    IF(Tair.GT.Tcold .and. Tair .LT. (Tcold+10.) .and. onset .eq. 0) beta_T=(Tair-Tcold)/10.
        !IF(Tair.LE.Tcold) beta_T=0.
    !    Write(*,*)'2beta_T',beta_T,Tair,Tcold,(Tair-Tcold)/10.
        !stop
    !ENDIF

    ! correct code - wan -Aug 13

    !IF (Tair.GT.Tcold .and. Tair .LT. (Tcold+10.)) THEN
    !    beta_T=(Tair-Tcold)/5.!10.  wan change 10 to 5
    !    beta_T = AMAX1(0.,AMIN1(1., beta_T))
    !ELSEIF (Tair.GE.(Tcold+10.)) THEN
    !    beta_T=1.
    !ELSEIF (Tair.LE.Tcold) THEN
    !    beta_T=0.
    !ENDIF

    beta_T = 1
    !First stage  onset = 0 low T
    !IF(Tair .LE. (Tcold+10))THEN
    !    beta_T= 0.  
        !write(*,*)'0betaT',beta_T,itime,onset
    !Second stage, rapid growth, suitable temperature
    !ELSEIF(Tair .GT. (Tcold+15) .and. Tair .LT. (Tcold+20.) .and. itime .LT. Tairmax_Loc_value )THEN
    !    beta_T = 1.
        !write(*,*)'1betaT',beta_T,itime,onset
    !Third stage, keep，but a litter decrease because Too hot, 
    !ELSEIF(Tair .ge. (Tcold+20.).and. Tair .LE. Tairmax)THEN
    !    beta_T=(Tair-Tcold)/20.  !100
    !    beta_T = AMAX1(0.,AMIN1(1., beta_T))
        !write(*,*)'1beta_T',beta_T,itime,onset,Tairmax_Loc_value,Tair
    !Fourth stage, prepare for winter 
    !ELSEIF(Tair .GT. (Tcold+10) .and. Tair .LT. Tairmax .and. itime .GT. Tairmax_Loc_value )THEN
    !    beta_T=(Tair-Tcold)/10.!10.  wan change 10 to 5
    !    beta_T = AMAX1(0.,AMIN1(1., beta_T))
        !write(*,*)'3beta_T',beta_T,itime,onset
    !ENDIF
...
!Fourth stage, prepare for winter 
    ...
    !WRITE(*,*)'gamma_WT',gamma_W,gamma_T
    ...
    gamma_N = exitK(1)
    !write(*,*)'1',exitK(1),1./tauC(1)
    IF (exitK(1) .NE. 1./tauC(1))THEN
        write(*,*)'exitK(1) .NE. 1./tauC(1)',exitK(1),1./tauC(1)
        STOP
    ENDIF
    ...
    !WRITE(*,*)'gamma_N,tauC(1)',gamma_N,tauC(1)
    ...
    ! IF(gamma_W .GT. 0.0)THEN
    !     WRITE(*,*)'water limitation'
    !     !stop
    ! ENDIF
    L_tau_new = gamma_W+gamma_T+gamma_N !wan 20230708
    L_fall = QC(1)*L_tau_new!*(gamma_W+gamma_T+gamma_N) 
    !WRITE(*,*)'tau,W,T',gamma_N,gamma_W,gamma_T
    !WRITE(*,*)'1 L_fall,tau',L_fall,L_tau_new
    !L_fall = QC(1)*(gamm  a_W+gamma_T)  !use OC(1) or bmleaf?

    !gamma_N = 1.0/tauC(3)
    !WRITE(*,*)'tau(3)',tauC(3)
    !R_fall = QC(3)*(gamma_W+gamma_T+gamma_N)


      ! ! add by zhoujian: S_fall and R_fall for stem and root
      !   IF (tauC(2) < 8760.)THEN
      !       gamma_N = 1.0/(tauC(2)*Sw)
      !   ENDIF
      !   IF(LAI < LAIMIN) THEN
      !       gamma_W = 0.
      !       gamma_T = 0.
      !       gamma_N = 0.
      !   ENDIF
      ! !   S_fall = bmStem*0.45*AMIN1((gamma_W+gamma_T+gamma_N),0.99)
      !   S_fall = bmStem*0.45*gamma_N
      !   WRITE(*,*) "tauC:",tauC
      !   IF (tauC(3) < 8760.)THEN
      !       gamma_N = 1.0/(tauC(3)*Sw)
      !   ENDIF
      !   IF(LAI < LAIMIN) THEN
      !       gamma_W = 0.
      !       gamma_T = 0.
      !       gamma_N = 0.
      !   ENDIF
      ! !   R_fall = bmRoot*0.45*AMIN1((gamma_W+gamma_T+gamma_N),0.99)
      !   R_fall = bmRoot*0.45*gamma_N
    RETURN
    END
!************************************************************************
! carbon transfer module
! wan- Wang & Luo. 2008
SUBROUTINE TCS_CNP(Tair,Tsoil,runoff,&
    &               L_fall,S_fall,R_fall,&  ! add by zhoujian : S_fall and R_fall
    &               CNmin,CNmax,NSNmax,NSNmin,&        ! nitrogen
    &               SNvcmax,SNgrowth,SNRauto,SNrs,&
    &               P_leach) 
    ...
    REAL L_fall,L_add,S_fall, R_fall  ! add by zhoujian : S_fall and R_fall 
...
!---------------------------------------------------------------------------
    ...
    !WRITE(*,*)itime,'---------------Now call the TSC_CN-----------------------',itime
...
!   Max and min NSN pool
    ...
    !wan- why multiple the 0.3
    S_omega=S_w_min + (1.-S_w_min) * Amin1(1.0,0.3*omega) !wan 2022/12/14s
    !WRITE(*,*)'omega',omega
    !S_omega=S_w_min + (1.-S_w_min) * Amin1(1.0,0.8*omega) !for TianTong   
    !WRITE(*,*)'S_omega',S_omega

    DO i=1,5
!       S_t(i)=Q10h(i)**((Tsoil-5.)/10.)   !Oak
        S_t(i)=Q10h(i)**((Tsoil-10.)/10.)  !Duke  wan: use this equation according to GaoJie, 20230516
        !S_t(i)=Q10h(i)**(((Tsoil-15.)/10.))  !Tiantong
    ENDDO
...
!IF (xde22 .lt. 1)write(*,*)'xde22',xde22,N_net22,P_net22
 
!     the carbon leaving the pools
!-----------------------------------------------------------------
    !S_omega = 1. !if delete somega, what's the trend of RH?
    !S_t = 1.
...
!     heterotrophic respiration from each pool
    ...
    !QC_o = QC
    etaL2 = 1-etaL!0.6-etaL
    etaW2 = 1-etaW!0.8-etaWß
    etaR2 = 1-etaR!0.8-etaR
    etaRe2= 1-etaRe

!============================================================
!NP, Out
IF(CYCLE_CNP .GT. 1) THEN
!WRITE(*,*)'CALL N-cycle, STEP---N'
...
!NP, MINERALIZATION
    ...
    !CABLE : Decomposition rate of litter/soil * P pools of litter/soil
        ...
        !WRITE(*,*)'P_miner',P_miner
    ENDIF
!============================================================
!NP, IMMOBILIZATION
    CALL N_immobilization(N_imm2)
    !WRITE(*,*)'N_immob,N_miner,N_net',N_immob,N_miner,N_net
    !write(*,*)'1- QPlab',QPlab,P_net2
    IF(CYCLE_CNP .GT. 2.)THEN
        CALL P_immobilization(P_imm2)
    ENDIF
    !write(*,*)'2- QPlab',QPlab,P_net2

    !IF(itime .GT. 24) STOP
    ! Before plant uptake, we close an count  of net_min
    ! 2024 01 25, wan

    !IF(QNminer + N_net2 .LT. 0)THEN
    !N_imm2(:) = 0
    N_immob2 = N_imm2(1)+N_imm2(2)+N_imm2(3)
    N_net2 = N_miner2 - N_immob2
    !ENDIF

    !QNminer = QNminer + N_net2
    !write(*,*)'QNminer',QNminer


 
 !   2024 01 25, wan
 !   the first update of QPlab, QPlab-1
 !   Cant avoid the negative QPlab
     !IF(CYCLE_CNP .GT. 2.)THEN
         !write(*,*)'1- QPlab',QPlab,P_net2
     !    CALL SorbedPDynamicNew(Dsoillab_rec2)
         !write(*,*)'2- QPlab',QPlab,QPlab+P_net2
     !ENDIF
...
!IF(itime .EQ. 8760)THEN
    !write(*,*)'P_loss_rec',sum(P_loss_rec(:))
!    write(*,*)'P_miner2_rec,P_immob2_rec,P_net2_rec',sum(P_miner2_rec(:)),sum(P_immob2_rec(:)),&
!    & sum(P_net2_rec(:))
!    write(*,*)'P_dep_annual,Pwea,Fptase_rec,',Pdep*8760,Pwea*8760,sum(Fptase_rec(:))
    !write(*,*)'---------Dsoillab_rec,Dsoillab_rec2',sum(Dsoillab_rec(:)),sum(Dsoillab_rec2(:))
    !write(*,*)'P_in-P_out',(sum(P_net2_rec(:))+Pdep*8760+Pwea*8760+sum(Fptase_rec(:)))- &
    !& (sum(P_uptake_rec(:))+sum(P_loss_rec(:))+ sum(asorb_rec(:)))
!    write(*,*)'P_in_out_rec', sum(P_in_out_rec(:))
!ENDIF
...
! update carbon pools, ! steply change of each pool size
    ...
    IF(CYCLE_CNP >2) THEN
        ...
        ! & - Scalar_P_flow*QP(6)*LDOP0  
        ! Scalar_P_flow,LDON0 -- for leaching Dissoved N/P
...
!     aerodynamic resistance                                                
      ...
      !WRITE(*,*)'raero',raero   !IF(wind.LT.0.01) wind=0.01
...
!           leaf stomata-photosynthesis-transpiration model - daytime
     ...
     &               XX_rec,XX_med_rec,gscx_rec,gscx_med_rec)
        ...
        !IF(XX_med_rec .GT. 1E6)THEN
         !   write(*,*)'XX_med,Dleaf,radabv',XX_med_rec,Dleaf,radabv
            !stop
        !ENDIF
...
!
         ...
         !WRITE(*,*)'Aleaf 1 2',Aleaf(1),Aleaf(2)
...
!IF (itime .GT.4224 .and. itime .LT.4249)THEN
!        WRITE(*,*)'Tleaf1,Tleaf2',Tleaf1,Tleaf2
!ENDIF
...
!          boundary layer conductance for CO2 - single side only (mol/m2/s)
        ...
        !IF(Dleaf .EQ. 0)THEN
        !    write(*,*)'Here',XX_med_rec,Dair,Dleaf,Rnstar
        !ENDIF
!          choose smaller of Ac, Aq
           !WRITE(*,*)'CALL photosyn Aleafx', Aleafx           
...
!     calculate  Vcmax, Jmax at leaf temp using Reed et al (1976) function J appl Ecol 13:925
      ...
      !IF(itime .GT.4224 .and.itime.LT.4249)THEN
      !  WRITE(*,*)'TminV,gddonset',TminV,gddonset
      !ENDIF
      !WRITE(*,*)gddonset
      TmaxV=50.
      ToptV=35.
...
!ENDIF
      ...
      !IF (itime .GT.4224 .and. itime .LT.4249)THEN
      !      WRITE(*,*)'in photosyn ------ VcmxT,eJ,itime',VcmxT,itime
      !      WRITE(*,*)'in VJtemp Tlf,TminV,TmaxV,TopV,Vcmx1',&
      !      & Tlf,TminV,TmaxV,TopV,Vcmx1
      !      WRITE(*,*)'heeee tlf-tmin',Tlf-TminV
            !WRITE(*,*)'Tlf-TminVJ,pwr',Tlf-TminVJ,pwr
      !ENDIF


!WRITE(*,*)'VcmxT',VcmxT
!WRITE(*,*)'Tlf,TminV,TmaxV,ToptV,Vcmx1',Tlf,TminV,TmaxV,ToptV,Vcmx1
...
!     calculate XX using Lohammer model, and scale for soil moisture
      ...
      !IF(XX_med .GT. 1E6)THEN
      !  write(*,*)'Gs from MED equation too big since the small VPD at leaf surface'
      !  write(*,*)'2--XX_med,Dleafx,co2csx',XX_med,Dleafx,co2csx,SQRT(Dleafx),itime
      !  write(*,*)'1+g1_med/(SQRT(Dleafx)),co2csx',+g1_med/(SQRT(Dleafx)),co2csx
      !  write(*,*)'1.6*(1+g1_med/(SQRT(Dleafx)))/co2csx',1.6*(1+g1_med/(SQRT(Dleafx)))/co2csx
        !stop
      !ENDIF
...
!    calculate coefficients for quadratic equation for ci
      ...
      !IF (itime .GT.4224 .and. itime .LT.4249)THEN
      !      WRITE(*,*)'in photosyn ------ VcmxT,eJ,itime',VcmxT,eJ,itime
      !     WRITE(*,*)'in VHtemp Tlf,TminVJ,TmaxVJ,TopVJ,VJmax0',& 
      !     & Tlf,TminVJ,TmaxVJ,TopVJ,VJmax0
            !WRITE(*,*)'Tlf-TminVJ,pwr',Tlf-TminVJ,pwr
      !ENDIF

!     choose smaller of Ac, Aq
      sps=AMAX1(0.001,sps)                  !Weng, 3/30/2006
      !WRITE(*,*)'Acx,Aqx',Acx,Aqx
...
REAL function VJtemp(Tlf,TminVJ,TmaxVJ,ToptVJ,VJmax0)
      !IF(Tlf.LT.TminVJ)THEN
      !   Tlf=TminVJ
      !   WRITE(*,*)'Tlf < TminVJ',Tlf
      !ENDIF   !constrain leaf temperatures between min and max
...
SUBROUTINE costFObsCH4(Simu_dailyCH4,obs_CH4_MEMCMC,std_CH4,    &
                    &   len3,J_last,upgraded)

    IMPLICIT NONE
    REAL Simu_dailyCH4(6,250000)
    REAL obs_CH4_MEMCMC(2,1000),std_CH4(2,1000)
    INTEGER day,len3,i,upgraded
    REAL J_new,J_last,delta_J,J_CH4flux,j1,r_num
    REAL dObsSim

    J_CH4flux=0.0
    j1=0
	
    DO i=1,len3			!len1=m undefined here      m is the length of simu_dailyflux
    ...
    J_CH4flux=J_CH4flux/REAL(j1)

    J_new=J_CH4flux             ! IF multiple paras + J_..+J_..
    delta_J=J_new-J_last

    CALL random_number(r_num)

    IF(ISNAN(J_new))THEN        !QUESTION  not very clear
...
!    compute J_obs
	...
	!J_lai=0
	    !daily
	!std_obs_gpp=2.534!2.245  !wan change this according to TT 2021 observation
	!std_obs_er=1.572!0.615
	!std_obs_nee=2.307!2.424

    !sd value for original data, no NA value
    !2021_Cflux_hourly.csv
    
    ! wrong SD value match
	!std_obs_gpp=0.299
	!std_obs_er=0.067
	!std_obs_nee=0.287
	...
	!std_obs_gpp=0.299
	!std_obs_er=0.067
	!std_obs_nee=0.355 ! modified value

    ! hourly-close - RECO = GPP+NEE,IF GPP+NEE<0, original data = NA
    ! use half hourly data deduced the NA value
    ! hourly data calculated from the half hourly data
    ! HourlyCFlux_2021_close_999_use.txt
	!std_obs_gpp=0.277
	!std_obs_er=0.150
	!std_obs_nee=0.355
!     GPPER _ interp
...
!	WRITE(*,*)'cost=',J_gpp,J_nee,J_er,J_lai,J_biomass
    ...
    J_new=J_new*600   !wan  1000 for nee, 10 for er+gpp
    !write(*,*)'J',J_nee,J_new,J_last
    ! if is 10, the accepted rate is too high
    ! if is 1000, the accepeted rate is too low, very very low
    IF(isimu .eq. 1) J_last = J_new  ! wan, avoid infinity at the first simulation
    !WRITE(*,*)'J_new,J_gpp,er,nee',J_new,J_gpp,J_er,J_nee   
    !J_new=J_soilc 
...
!     delta_J=-1     !accept all samples, No data
    ...
    IF (rejectnow .eq. 0) THEN
        ...
        ! IF(AMIN1(1.0,exp(-delta_J)).gt.0.95)THEN  ! wan change this accepet probability
            upgraded=upgraded+1
            J_last=J_new    
            !write(*,*)'2-isimu,upgraded',isimu,upgraded
        ENDIF
    ENDIF
